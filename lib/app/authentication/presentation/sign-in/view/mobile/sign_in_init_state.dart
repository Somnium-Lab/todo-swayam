import 'package:flutter/material.dart';
import 'package:flutter_todo_clean_arch/app/authentication/presentation/sign-in/sign_in_controller.dart';
import 'package:flutter_todo_clean_arch/app/authentication/presentation/sign-in/view/widget/sign_in_content_body.dart';

class SignInMobileInitView extends StatefulWidget {
  final SignInController controller;

  const SignInMobileInitView({Key? key, required this.controller}) : super(key: key);

  @override
  _SignInMobileInitViewState createState() => _SignInMobileInitViewState();
}

class _SignInMobileInitViewState extends State<SignInMobileInitView> {

  late TextEditingController _emailTextController;
  late TextEditingController _passwordTextController;

  @override
  void initState() {
    _emailTextController = new TextEditingController();
    _passwordTextController = new TextEditingController();
    super.initState();
  } 

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(),
        body: Container(
            padding: EdgeInsets.symmetric(horizontal: 20, vertical: 30),
            child: SignInContentBody(
                email: _emailTextController,
                password: _passwordTextController,
                controller: widget.controller)));
  }
}